...
int print_instruction(SDB &sdb, long long addr, instruction1 *in);
int disassemble(SDB &sdb, unsigned long long rip);
...
{    
    ...
    { // loaded and running
        if(!LOADED)
        {
            fprintf(stderr, "** The program is not loaded.\n");
            return ;
        }
    ...
    else
    {
        ...
        { // not loaded
            if(LOADED)
            {
                fprintf(stderr, "** program '%s' has been loaded. entry point 0x%llx\n", sdb.program_path.c_str(), sdb.entry_point);
                return ;
            }
            
            if(script_vector.size() < 2)
            {
                fprintf(stderr, "** The command \'%s\' need %s.\n", "load", "path to a program");
                // implement_help(sdb);
                return ;
            }
            // clear sdb
        ...
        {
            fprintf(stderr, "** There are no command named \'%s\'.\n", script_vector[0].c_str());
        }
    }
}
...
void implement_break(SDB &sdb, unsigned long long instruction_address)
{
    ...
    {
        fprintf(stderr, "** [break] PTRACE_POKETEXT error.\n");
        implement_exit(sdb);
    }
...
void implement_cont(SDB &sdb)
{
    ...
    {
        fprintf(stderr, "** [cont] PTRACE_GETREGS error.\n");
        implement_exit(sdb);
    }
    ...
    {
        fprintf(stderr, "** [cont] cont error.\n");
        implement_exit(sdb);
    }
    
    if(waitpid(sdb.child, &sdb.status, 0) < 0)
    {
        fprintf(stderr, "** [cont] waitpid error.\n");
        implement_exit(sdb);
    }
    ...
    else
    {
        ...
        {
            fprintf(stderr, "** [cont] PTRACE_PEEKTEXT error.\n");
            implement_exit(sdb);
        }
        ...
        if(it != sdb.break_addrs.end())
        {
            ...
            {
                fprintf(stderr, "** [cont] PTRACE_POKETEXT error.\n");
                implement_exit(sdb);
            }
            
            if(ptrace(PTRACE_SETREGS, sdb.child, 0, &regs))
            {
                fprintf(stderr, "** [cont] PTRACE_SETREGS error.\n");
                implement_exit(sdb);
            } 
            ...
            if(sdb.mi == sdb.m.end()) {
                sdb.m.clear();
                load_maps(sdb.child, sdb.m);
                // fprintf(stderr, "## %zu map entries re-loaded.\n", sdb.m.size());
                sdb.mi = sdb.m.find(r);
            }
            fprintf(stderr, "** breakpoint @ ");
            disassemble(sdb, regs.rip);
...
void implement_delete(SDB &sdb, int break_point_id)
{
    ...
    {
        fprintf(stderr, "** breakpoint %d is not exist.\n", break_point_id);
    }
    ...
    else
    {
        if(sdb.re_break_index == -1)
        {
            ...
            {
                fprintf(stderr, "** [delete] PTRACE_PEEKTEXT error.\n");
                implement_exit(sdb);
            }
            ...
            {
                fprintf(stderr, "** [delete] PTRACE_POKETEXT error.\n");
                implement_exit(sdb);
            }
        }
...
void implement_dump(SDB &sdb, unsigned long long addr, int length)
{
    ...
    while(true)
    {
        ...
        {
            fprintf(stderr, "** [dump] PTRACE_PEEKTEXT error.\n");
            implement_exit(sdb);
        }
...
void implement_get(SDB &sdb, std::string reg)
{
    ...
    if(WIFSTOPPED(sdb.status))
    {
        ...
        if(ptrace(PTRACE_GETREGS, sdb.child, 0, &regs) != 0)
        {
            fprintf(stderr, "** [get] PTRACE_GETREGS error.\n");
            implement_exit(sdb);
        }
        else
        {
            ...
            if(val == -1)
                fprintf(stderr, "** There are no reg named \'%s\'.\n", reg.c_str());
            else
...
void implement_getregs(SDB &sdb)
{
    ...
    if(WIFSTOPPED(sdb.status))
    {
        ...
        if(ptrace(PTRACE_GETREGS, sdb.child, 0, &regs) != 0)
        {
            fprintf(stderr, "** [getregs] PTRACE_GETREGS error.\n");
            implement_exit(sdb);
        }
        else
        {
...
void implement_load(SDB &sdb, std::string path)
{
    ...
    else
    {
        ...
        sdb.entry_point = elf.e_entry;
        
        /*
        fprintf(stderr, "[elf] %llx.\n", elf.e_type);
        fprintf(stderr, "[elf] %llx.\n", elf.e_machine);
        fprintf(stderr, "[elf] %llx.\n", elf.e_version);
        fprintf(stderr, "[elf] %llx.\n", elf.e_entry);
        fprintf(stderr, "[elf] %llx.\n", elf.e_phoff);
        ...
        {
            fprintf(stderr, "** [load] cs_open error.\n");
            implement_exit(sdb);
        }
...
void implement_set(SDB &sdb, std::string reg, unsigned long long val)
{
    ...
    if(WIFSTOPPED(sdb.status))
    {
        ...
        if(ptrace(PTRACE_GETREGS, sdb.child, 0, &regs) != 0)
        {
            fprintf(stderr, "** [set] PTRACE_GETREGS error.\n");
            implement_exit(sdb);
        }
        else
        {
            ...
            else if(old_val != val)
            {
                ...
                {
                    fprintf(stderr, "** [set] PTRACE_POKETEXT error.\n");
                    implement_exit(sdb);
                }
            }
        }
...
bool implement_si(SDB &sdb)
{
    ...
    if(ptrace(PTRACE_GETREGS, sdb.child, 0, &regs) != 0)
    {
        fprintf(stderr, "** [si] PTRACE_GETREGS error.\n");
        implement_exit(sdb);
    }
    ...
    {
        fprintf(stderr, "** [si] PTRACE_SINGLESTEP error.\n");
        implement_exit(sdb);
    }
    
    if(waitpid(sdb.child, &sdb.status, 0) < 0)
    {
        fprintf(stderr, "** [si] waitpid error.\n");
        implement_exit(sdb);
    }
    ...
    {
        // if there are break index need to reset, check if in same rip then implement
        ...
        else
        {
            ...
            if(it != sdb.break_addrs.end() && sdb.re_break_index == -1)
            {
                ...
                if((code = ptrace(PTRACE_PEEKTEXT, sdb.child, regs.rip, 0)) < 0)
                {
                    fprintf(stderr, "** [si] PTRACE_PEEKTEXT error.\n");
                    implement_exit(sdb);
                }
                ...
                if(ptrace(PTRACE_POKETEXT, sdb.child, regs.rip, code ^ 0xcc | sdb.break_code[index]) < 0)
                {
                    fprintf(stderr, "** [si] PTRACE_POKETEXT error.\n");
                    implement_exit(sdb);
                }
                
                if(ptrace(PTRACE_SETREGS, sdb.child, 0, &regs))
                {
                    fprintf(stderr, "** [si] PTRACE_SETREGS error.\n");
                    implement_exit(sdb);
                } 
...
void implement_start(SDB &sdb)
{
    ...
    // error
    if(sdb.child < 0){
        fprintf(stderr, "** [start] fork error.\n");
        implement_exit(sdb);
    }
    ...
    if(sdb.child == 0)
    {
        ...
        {
            fprintf(stderr, "** [start] PTRACE_TRACEME error.\n");
            implement_exit(sdb);
        }
        
        execvp(sdb.program_path.c_str(), sdb.program_args.data());
        fprintf(stderr, "** [start] \'%s\' execute error.\n", sdb.program_path.c_str());
        implement_exit(sdb);
    }
    else
    {
        if(waitpid(sdb.child, &sdb.status, 0) < 0)
        {
            fprintf(stderr, "** [start] waitpid error.\n");
            implement_exit(sdb);
        }
...
int disassemble(SDB &sdb, unsigned long long rip)
{
    ...
    {
        return print_instruction(sdb, rip, &mi->second);
    }
    ...
    if(ptr == rip)
    {
        return print_instruction(sdb, rip, NULL);
    }
