...
void implement_delete(SDB &sdb, int break_point_id, bool recover);
...
{    
    ...
    else
    {
        ...
        { // running
            ...
            implement_delete(sdb, string_to_int(script_vector[1]), false);
        }
...
void implement_cont(SDB &sdb)
{
    ...
    // if next step is break point, then return
    if(implement_si(sdb))
        return ;
    ...
    {
        fprintf(stdout, "** child process %d terminiated normally (code %d)\n", sdb.child, WEXITSTATUS(sdb.status));
        LOADED = false;
        RUNNING = false;
    }
    else
    {
        ...
        // restore code by delete with recover variable
        auto it = std::find(sdb.break_addrs.begin(), sdb.break_addrs.end(), regs.rip);
        if(it != sdb.break_addrs.end())
        {
            int index = it - sdb.break_addrs.begin();
            if(sdb.break_code[index] != 0xffffffff)
            {
                implement_delete(sdb, index, true);
...
void implement_delete(SDB &sdb, int break_point_id, bool recover)
{
    ...
    else if(sdb.break_code[break_point_id] == 0xffffffff)
    {
        fprintf(stdout, "** breakpoint %d is not exist.\n", break_point_id);
    }
    else
    {
        if(sdb.break_code[break_point_id] != 0xffff)
        {
            // get code in specify address
            unsigned long long code;
        ...
        sdb.break_code[break_point_id] = 0xffff;
        
        // delete break point
        if(!recover)
        {
            sdb.break_code[break_point_id] = 0xffffffff;
            if(sdb.re_break_index == break_point_id)
                sdb.re_break_index = -1;
...
void implement_list(SDB &sdb)
{
    ...
    {
        if(sdb.break_code[i] != 0xffffffff)
    ...
    if(WIFSTOPPED(sdb.status))
    {
        ...
        else
        {
            ...
            else if(old_val != val)
            {
                ...
                // if change rip and there are break index need to reset
                if(reg == "rip" && sdb.re_break_index != -1 && sdb.break_code[sdb.re_break_index] == 0xffff)
                {
...
bool implement_si(SDB &sdb)
{
    ...
    // get rip and code, check if next step is break point
    struct user_regs_struct regs;
    if(ptrace(PTRACE_GETREGS, sdb.child, 0, &regs) != 0)
    {
    ...
    if(temp == 0xcc)
    {
        ...
        {
            fprintf(stdout, "** [si] break point error.\n");
            implement_exit(sdb);
        }
    }
    ...
    else
    {
        ...
        if(sdb.re_break_index != -1 && sdb.break_code[sdb.re_break_index] == 0xffff)
        {
            implement_break(sdb, sdb.break_addrs[sdb.re_break_index]);
            sdb.re_break_index = -1;
        }
